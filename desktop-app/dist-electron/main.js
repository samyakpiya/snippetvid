import { Menu, ipcMain, app, desktopCapturer, net, BrowserWindow } from "electron";
...
const httpsClient = axios.create({
  baseURL: "https://snippetvid.com/api",
...
const defaultWindowConfig = {
  ...
  resizable: true,
  skipTaskbar: true,
  icon: path$1.join(process.env.VITE_PUBLIC, "SnippetVid Logo.svg"),
  webPreferences: {
    preload: path$1.join(__dirname, "preload.mjs")
  }
};
function loadWindowContent(window2, pageName) {
  if (VITE_DEV_SERVER_URL) {
    const devUrl = pageName === "index" ? VITE_DEV_SERVER_URL : `${"http://localhost:5173"}/${pageName}.html`;
    window2.loadURL(devUrl);
  } else {
    window2.loadFile(path$1.join(RENDERER_DIST, `${pageName}.html`));
  }
}
function createWindow() {
  ...
  {
    floatingWebCam == null ? void 0 : floatingWebCam.webContents.openDevTools();
...
ipcMain.handle("fetch-user-data", async (_event, clerkId) => {
  console.log("Fetching user data for:", clerkId);
  const request = net.request(
    `${"https://snippetvid.com/api"}/auth/${clerkId}`
  );
...
function ensureWindowsOnTop() {
  [win, studio, floatingWebCam].forEach((window2) => {
